...
const CartSidebar = ({ open, onClose, items, onUpdateQty, onRemove, onClear }: CartSidebarProps) => {
  const grouped = items.reduce<{ product: Product; qty: number }[]>((acc, item) => {
    const found = acc.find(r => r.product.id === item.id);
            ...
            <div className="cart-items">
              {grouped.map(({ product: p, qty }) => (
                <div className="cart-item" key={p.id}>
                  <div className="cart-item__img">{p.emoji}</div>
                  <div className="cart-item__info">
                    ...
                    <div className="cart-item__bottom">
                      ...
                      <div className="qty-stepper">
                        <button className="qty-btn" onClick={() => onUpdateQty(p.id, -1)} disabled={qty <= 1}>−</button>
                        <span className="qty-val">{qty}</span>
                        <button className="qty-btn" onClick={() => onUpdateQty(p.id, 1)}>+</button>
                      </div>
                      <button className="cart-item__remove" onClick={() => onRemove(p.id)} title="Remove item">🗑</button>
                    </div>
                  </div>
                </div>
              ))}
